...
public class CardDestroy2 : MonoBehaviour
{
    ...
    private void OnMouseDown() //Función para que el jugador pueda clicar sobre una carta y "voltearla"
    {
        ...
        {
            turno.enabled = true;
            ran = random.GetComponent<GameManagerMedium>().random; //Variable que recoge el valor de la varaible random del Script GameManager para saber la posición de la carta corazón



            if (Value == ran) //Si Value es igual a random, entonces el minijuego se bloquea y sale un mensaje de +2UP, para que el jugador reciba una vida.
            {

                gameObject.SetActive(false);
                player = false;
                maquina.GetComponent<GameManagerMedium>().maquina = false;
                turno.text = " ";
                texto.text = "Victoria";
            ...
            {
                gameObject.SetActive(false);
                maquina.GetComponent<GameManagerMedium>().maquina = true;
                Check = true;
                player = false;
