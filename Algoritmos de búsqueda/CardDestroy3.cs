...
public class CardDestroy3 : MonoBehaviour
{
    ...
    private void OnMouseDown() //Función para que el jugador pueda clicar sobre una carta y "voltearla"
    {
        if (player == true)
        {
            ...
            ran = random.GetComponent<GameManagerHard>().random; //Variable que recoge el valor de la varaible random del Script GameManager para saber la posición de la carta corazón



            if (Value == ran) //Si Value es igual a random, entonces el minijuego se bloquea y sale un mensaje de +3UP, para que el jugador reciba una vida.
            {

                gameObject.SetActive(false);
                player = false;
                maquina.GetComponent<GameManagerHard>().maquina = false;
                turno.text = " ";
                texto.text = "Victoria";
            ...
            {
                gameObject.SetActive(false);
                maquina.GetComponent<GameManagerHard>().maquina = true;
                Check = true;
                player = false;
            }
